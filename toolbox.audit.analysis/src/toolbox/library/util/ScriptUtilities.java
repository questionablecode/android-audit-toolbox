...
public final class ScriptUtilities {
	...
	public static Q apis() {
		return declarations(Common.index().nodesTaggedWithAny(Node.LIBRARY), TraversalDirection.FORWARD).difference(arrayTypes(),
	...
	public static Q objectMethodOverrides() {
		return Common.edges(Edge.OVERRIDES).reverse(
				declarations(Common.typeSelect("java.lang", "Object"), TraversalDirection.FORWARD).nodesTaggedWithAny(Node.METHOD));
	}
	...
	public static String stringify(Q query, boolean qualified) {
		...
		for (GraphElement node : query.eval().nodes()) {
			sb.append(prefix);
			if (qualified) {
				sb.append(Common.getQualifiedName(node).trim());
			} else {
	...
	public static Q readersOf(Q context, Q origin) {
		return Common.stepTo(context.edgesTaggedWithAny(Edge.DF_LOCAL, Edge.DF_INTERPROCEDURAL), origin);
	}
	...
	public static Q writersOf(Q context, Q origin) {
		return Common.stepFrom(context.edgesTaggedWithAny(Edge.DF_LOCAL, Edge.DF_INTERPROCEDURAL), origin);
	}
